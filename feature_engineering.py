...
def load_and_prepare_data(path):
    ...
    print("📊 Merge macro principali (FEDFUNDS, GS10, GS2, DXY, 10Y-2Y)")
    df = df.merge(macro, how="left", left_on="Date", right_index=True)

    print("✅ Download VIX (^VIX) da Yahoo Finance...")
    vix_data = yf.download("^VIX", start=start_date, end=end_date, interval="1d", progress=False)
    
    print("📋 Colonne VIX:", vix_data.columns)
    print("📊 Tipo vix_data:", type(vix_data))
    print("📈 Esempio primi dati VIX:\n", vix_data.head())
    
    # Estrazione serie 'Close'
    try:
        if isinstance(vix_data.columns, pd.MultiIndex):
            ...
            raise ValueError("❌ Errore: 'Close' non trovato nei dati VIX")
    except Exception as e:
        print("❌ Errore nella selezione della colonna 'Close':", e)
        raise
    
    vix_series.name = "VIX"
    vix_series = vix_series.dropna()
    df = df.merge(vix_series, how="left", left_on="Date", right_index=True)
    print("📊 VIX da Yahoo: valori disponibili =", df["VIX"].notna().sum())

    # ✅ Feature derivate VIX
    ...
    vix_features = ["VIX_Z20", "VIX_Momentum_5d", "VIX_Change_1d"]
    feature_list += vix_features
    full_feature_list += vix_features
    print("✅ Feature derivate VIX aggiunte:", vix_features)
    dates_after = set(df["Date"])
    missing = original_dates - dates_after
    ...
    macro_columns = ["CPI", "CoreCPI", "NFP", "PPI", "RetailSales", "GDP_QoQ", "JOLTS", "FEDFUNDS", "GS10", "GS2", "DXY", "10Y-2Y"]
